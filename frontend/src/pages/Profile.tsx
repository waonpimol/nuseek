      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
            <h3 className="text-4xl font-bold text-orange-500">{itemsLoading ? "-" : totalCount}</h3>
            <p className="text-gray-500 mt-2">ประกาศทั้งหมด</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
            <h3 className="text-4xl font-bold text-red-500">{itemsLoading ? "-" : lostCount}</h3>
            <p className="text-gray-500 mt-2">ของที่หาย</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
            <h3 className="text-4xl font-bold text-sky-500">{itemsLoading ? "-" : foundCount}</h3>
            <p className="text-gray-500 mt-2">ของที่พบ</p>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
            <h3 className="text-4xl font-bold text-green-500">{itemsLoading ? "-" : matchedCount}</h3>
            <p className="text-gray-500 mt-2">พบเจ้าของแล้ว</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
              {filteredItems.map((item) => (
                <div
                  ...
                  className="border border-gray-100 rounded-2xl overflow-hidden hover:shadow-md transition cursor-pointer flex flex-col"
                >
                  <div className="relative aspect-[4/3] bg-gray-100">
                    ...
                    />
                    {item.status !== "matched" && (
                      <div className="absolute top-2 left-2 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm text-xs font-semibold">
                        <span className={`w-2 h-2 rounded-full ${item.type === "lost" ? "bg-red-500" : "bg-sky-500"}`} />
                        {typeLabel(item.type)}
                      </div>
                    )}
                    {item.status === "matched" && (
                      <div className="absolute top-2 left-2 bg-green-500 text-white px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm">
                        พบเจ้าของแล้ว
                      </div>
                    )}
                  </div>
                  <div className="p-4 space-y-2">
                    <h4 className="font-semibold text-gray-800 line-clamp-1">{item.title || "ไม่ระบุชื่อสิ่งของ"}</h4>
                    <div className="flex items-center gap-1.5 text-xs text-gray-500">
                      <MapPin size={13} />
                      <span className="line-clamp-1">{item.location || "ไม่ระบุสถานที่"}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-gray-500">
                      <Clock size={13} />
